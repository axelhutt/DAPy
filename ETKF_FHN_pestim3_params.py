...
class Kalman:
	def __init__(self):
		...
		duration = 200.0 
		self.dt_iteration  	= 0.05 # model integration time step 
		self.num_iteration 	= 200 # number of model iterations between two observation times
		self.dt           	= self.dt_iteration*float(self.num_iteration) # observation time step
		self.num_time     	= int(duration/self.dt)  # number of observation times = number of DA cycles
		
		
		self.t             	= 0  # count the loops in the model
		self.time          	= np.zeros(self.num_time)  # time points ob observations
		self.dim_obs       	= 2
		self.num_params    	= 1 # number of model parameters to be estimated 
	...
	def initialize_basic(self):
		...
		# false model
		self.I_1_false     = 20.0 # different parameter
		self.b0_1_false    = 2.0
		self.b1_1_false    = 1.3
		...
		self.I_1_min = 0.001
		self.tau_2_max = 20.0
		self.tau_2_min = 0.001
		
		self.model_noise   		= 0.8 # noise for FHN-model 
		self.initial_ensnoise 	= 0.1 # initial ensemble noise
		self.addcovinfl			= 0.05 # additive covariance inflation
		self.multensinfl			= 1.0 # multiplicative ensemble inflation factor
